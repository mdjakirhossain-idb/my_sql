...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 02, 2024 at 12:21 PM
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_product2`  AS SELECT `p`.`id` AS `id`, `p`.`name` AS `name`, `p`.`price` AS `price`, `m`.`name` AS `mname`, `m`.`id` AS `nid`, `m`.`contact_no` AS `contact_no` FROM (`product` `p` join `manufacturer` `m`) WHERE `p`.`manufacturer_id` = `m`.`id` AND `p`.`price` > 5000 ;
